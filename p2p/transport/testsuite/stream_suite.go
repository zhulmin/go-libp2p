...
func SubtestStress(t *testing.T, ta, tb transport.Transport, maddr ma.Multiaddr, peerA peer.ID, opt Options) {
	...
	openConnAndRW := func() {
		...
		c, err := GetDialResult(tb.Dial(context.Background(), l.Multiaddr(), peerA))
		if err != nil {
			t.Error(err)
			return
		}
...
func SubtestStreamOpenStress(t *testing.T, ta, tb transport.Transport, maddr ma.Multiaddr, peerA peer.ID) {
	...
	connB, err = GetDialResult(tb.Dial(context.Background(), l.Multiaddr(), peerA))
	if err != nil {
		t.Fatal(err)
	}
	err = <-accepted
...
func SubtestStreamReset(t *testing.T, ta, tb transport.Transport, maddr ma.Multiaddr, peerA peer.ID) {
	...
	muxb, err := GetDialResult(tb.Dial(context.Background(), l.Multiaddr(), peerA))
	if err != nil {
		t.Fatal(err)
	}
	defer muxb.Close()
